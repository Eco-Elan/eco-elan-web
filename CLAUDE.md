...
- **Server-only** modules are in **`src/server/`** (excluded from `tsconfig.app.json`; typechecked via `tsconfig.api.json` — run `npx tsc -p tsconfig.api.json`): `supabaseAdmin.ts` (service-role client + row↔Order mapping + data access), `adminAuth.ts` (`requireAdmin` = verify Supabase JWT + `ADMIN_EMAILS` allowlist — **the real authz boundary**; the client gate is UX), and `pdf/index.ts` (`@react-pdf/renderer` quotation/invoice/receipt PDFs, styled to match the design-system documents). Two gotchas baked in here: it's plain `.ts`/`createElement`, **not** `.tsx` (Vercel's bundler won't resolve a `.js` import to a `.tsx` source, so a `.tsx` module would be dropped from the deployed function); and the logo + Plus Jakarta Sans font are embedded as base64 in **`pdf/assets.ts`** (generated) so there's **no network fetch or file-tracing at render time** — a remote image/font fetch can crash the serverless render.
